...
def elastic(masses, speeds):
    ...
    else: raise ValueError("Function only works for two particle collisions in 1 dimension")
    
    P1, P2 = four_momenta[0:2]
    total_P = P1 + P2
    v_CoM = total_P[1]/total_P[0]
    #CoM components
    total_P_CoM = lorentz_boost(v_CoM, total_P)
    P1_CoM = lorentz_boost(v_CoM, P1)
    P2_CoM = lorentz_boost(v_CoM, P2)
    #After collision
    final_P1_CoM = np.diag([1, -1, 1, 1]) @ P1_CoM
    final_P2_CoM = np.diag([1, -1, 1, 1]) @ P2_CoM
    #Transform back to Lab frame
    final_P1 = lorentz_boost(-v_CoM, final_P1_CoM)
    final_P2 = lorentz_boost(-v_CoM, final_P2_CoM)
