...
def minkowski_diagram(events, vs, size, hyperbolae = False):
    fig, ax = plt.subplots(figsize = [6,6])
    ...
    for l in range(len(vs)):
        v = vs[l]
        if np.abs(v) <= 1:
            plt.plot(c, c/v, linestyle = ':', color = 'k') # x' axis has t' = 0
            plt.plot(c, v*c, linestyle = ':', color = 'k') # t' axis has x' = 0
        else:
            raise ValueError(f"velocity v={v} must satisfy |v| <= 1.")
        
    if hyperbolae == True:
        plt.plot(c, np.sqrt(c**2-1), color = 'k', linestyle = '--', label = 'Calibration hyperbolae')
...
tester_v = [0.5]
minkowski_diagram(tester, tester_v, 8, False)
